...
function authKnownSubFeatures()
{
    return array(
        AclCompetition => array(
            'cData' => 'Competition data',
            'cSchedule' => 'Schedule / sessions',
            'cFinalReport' => 'Final report',
            'cExport' => 'Exports',
        ),
        AclParticipants => array(
            'pEntries' => 'Entries',
            'pAdvancedEntries' => 'Advanced entries',
            'pTarget' => 'Target assignment',
            'pAdvancedTarget' => 'Advanced target assignment',
        ),
        AclAccreditation => array(
            'acStandard' => 'Accreditation',
            'acSetup' => 'Accreditation setup',
        ),
        AclInternetPublish => array(
            'ipCredentials' => 'Publish credentials',
            'ipSend' => 'Send online',
        ),
        AclISKServer => array(
            'iskManagement' => 'ISK management',
            'iskUser' => 'ISK users',
        ),
        AclModules => array(
            'Authentication' => 'Authentication module',
            'competplus' => 'Compet+ module',
            'Barcodes' => 'Barcodes module',
            'Average' => 'Average module',
            'UpdateWeb' => 'UpdateWeb module',
        ),
        AclAPI => array(
            'apiRead' => 'API read',
            'apiWrite' => 'API write',
...
function authAccessLabel($featureString)
{
    if (trim((string)$featureString) === '') {
        return 'Tournament visibility only';
    }
    $readOnly = 0;
    $readWrite = 0;
    $deniedSub = 0;
    ...
    $suffix = $deniedSub ? ' / ' . $deniedSub . ' denied subfeature' . ($deniedSub > 1 ? 's' : '') : '';
    return $readWrite . ' write / ' . $readOnly . ' read' . $suffix;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($user === '') {
        ...
        if ($pattern === '') {
            $error = 'Competition pattern is required.';
        } else {
...
<div class="cp-admin-shell">
    <div class="cp-admin-hero">
        <div class="cp-admin-brand">
            ...
            <div>
                <h1 class="cp-admin-title">Authentication admin</h1>
                <p class="cp-admin-subtitle">User access, tournament visibility and fine-grained ACL subfeatures.</p>
            </div>
        </div>
        <div class="cp-admin-credit">Module made available by<br><a href="https://competplus.fr" target="_blank" rel="noopener noreferrer">Compet+</a></div>
    </div>
    <?php if (!empty($_SESSION['AUTH_User'])) { ?><p>Logged as <b><?php echo htmlspecialchars($_SESSION['AUTH_User']); ?></b> — <a class="Link" href="LogOut.php">Logout</a></p><?php } ?>
    ...
    <p class="cp-license-note">This open-source module is provided by <a href="https://competplus.fr" target="_blank" rel="noopener noreferrer">Compet+</a>. Reuse is allowed under the included license, with copyright notice and attribution preserved.</p>
        ...
        <div class="auth-card">
            ...
            <?php foreach ($users as $u) { ?>
                <a class="auth-user" href="?user=<?php echo urlencode($u->AclUsUser); ?>">
                    ...
                    <span class="auth-badge"><?php echo intval($u->AclUsEnabled) ? 'enabled' : 'disabled'; ?></span>
                    <?php if (intval($u->AclUsAuthAdmin)) { ?><span class="auth-badge">root</span><?php } ?>
                </a>
            <?php } ?>
        </div>

        <div class="auth-card">
            <h2><?php echo $selectedUserRow ? 'Edit user' : 'Create user'; ?></h2>
            <form method="post">
                <input type="hidden" name="action" value="save_user">
                <table class="Tabella">
                    <tr><td>User</td><td><input name="user" maxlength="16" required value="<?php echo htmlspecialchars($selectedUserRow->AclUsUser ?? ''); ?>"> <small>letters, digits, dot, dash, underscore</small></td></tr>
                    <tr><td>Name</td><td><input name="name" maxlength="100" value="<?php echo htmlspecialchars($selectedUserRow->AclUsName ?? ''); ?>"></td></tr>
                    <tr><td>Password</td><td><input type="password" name="password" autocomplete="new-password"> <small>leave empty to keep current password</small></td></tr>
                    <tr><td>Enabled</td><td><input type="checkbox" name="enabled" <?php echo (!$selectedUserRow || intval($selectedUserRow->AclUsEnabled)) ? 'checked' : ''; ?>></td></tr>
                    <tr><td>Admin/root</td><td><input type="checkbox" name="admin" <?php echo ($selectedUserRow && intval($selectedUserRow->AclUsAuthAdmin)) ? 'checked' : ''; ?>> <small>root users bypass all tournament rules</small></td></tr>
                    <tr><td colspan="2" class="Center"><input type="submit" value="Save user"></td></tr>
                </table>
            </form>
            <?php if ($selectedUserRow) { ?>
            <form method="post" onsubmit="return confirm('Delete this user and all access rules?')">
            ...
            <?php if ($selectedUserRow) { ?>
                <h2>Access rules</h2>
                <table class="Tabella">
                    <tr><th>Competition pattern</th><th>Access summary</th><th>Raw rule</th><th>Actions</th></tr>
                    <?php foreach ($selectedRules as $rule) { ?>
                        <tr>
                            ...
                            <td><small><?php echo htmlspecialchars($rule['features'] === '' ? 'visibility only' : $rule['features']); ?></small></td>
                            <td>
                                <form method="post" style="display:inline" onsubmit="return confirm('Delete this access rule?')">
                ...
                <form method="post">
                    ...
                    <table class="Tabella">
                        <tr>
                            ...
                            <td>
                                <select name="tournament_code">
                                    <option value="">Use manual pattern below</option>
                        ...
                        <tr><td>Manual pattern</td><td><input name="pattern" placeholder="TOURCODE, FR%, FR*, *" maxlength="150"> <small><code>*</code> gives access to all competitions</small></td></tr>
                    </table>
                    <table class="auth-permissions">
                        <tr><th>Feature / subfeature</th><th class="Center">No access</th><th class="Center">Read only</th><th class="Center">Read/write</th></tr>
                        <?php
                        global $listACL;
                    ...
                    <p><input type="submit" value="Save access rule"></p>
                </form>
            <?php } ?>
        </div>
    </div>
